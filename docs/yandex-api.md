...
| Пунктуация | **Отсутствует** на русской речи: сплошной текст строчными буквами, без точек и запятых, несмотря на `literature_text: true`. Для английской речи не проверено | `merge_phrases` не видит конца предложения (`SENTENCE_END`) и склеивает соседние сегменты всегда, пока зазор ≤ 0,8 с и длина ≤ 12 с. Перевод получает текст без пунктуации |
