...
Проверено по официальной документации **11.09.2026**. Раздел «Реальные ответы» заполнен по двум
прогонам пользователя 11.09.2026 (по 60 с видео, `--fast`, `language_code: auto`): русская речь и
английская. При расхождении с реальным
ответом правы документация и реальный ответ — обнови этот файл и эмуляцию в `tests/test_yandex.py`.
...
| Язык | `language_code: ["auto"]` — по документации определение по каждому предложению; фактически в ответе приходит распределение вероятностей на весь `final`, см. «Реальные ответы» | [Языки распознавания](https://aistudio.yandex.ru/docs/ru/speechkit/stt/models.html) |
...
| Тариф | Стандартный 0,1515 ₽, отложенный 0,0381 ₽ за 15 с с НДС; посекундно с 16-й секунды; моно считается как 2 канала. ⚠ оценка в коде множитель за каналы не применяет, см. «Реальные ответы» | [Тарифы SpeechKit](https://aistudio.yandex.ru/ru/docs/speechkit/pricing) |
...
Источник: два прогона 11.09.2026 по 60 с, модель `general`, `language_code: ["auto"]`,
`literature_text: true` — русская речь (закадровый перевод лекции) и английская речь (оригинал
лекции). Дословные ответы сохраняются в `<имя>_work/stt_raw.txt`, эмуляция — `recognition_real()`
в `tests/test_yandex.py` (варианты `ru` и `en`).

| Факт | Наблюдение | Последствие для кода |
|---|---|---|
| Нарезка `final` | Крупными блоками, не по фразам, и шаг непостоянный: на русской речи два блока (0–30 660 мс и 30 660–60 000 мс), на английской — **один на все 59 800 мс**. Похоже, границу задаёт достаточно длинная пауза, а не фиксированный интервал | Резать самим: `split_long` по паузам между словами, порог `MAX_SEG`. Рассчитывать на блоки определённой длины нельзя |
| Пунктуация | **Отсутствует на обоих языках**: сплошной текст строчными буквами, без точек и запятых, несмотря на `literature_text: true` | `merge_phrases` не видит конца предложения (`SENTENCE_END`) и склеивает соседние сегменты всегда, пока зазор ≤ 0,8 с и длина ≤ 12 с. Перевод получает текст без пунктуации |
...
| `languages` при `auto` | Не список найденных языков, а **распределение вероятностей** по языкам модели. Русская речь: 13 записей, ru-RU 0,751; kk-KK 0,221; en-EN 0,025. Английская: 12 записей (kk-KK отсутствует), en-EN 0,827; остальные < 0,03. Набор языков от ответа к ответу меняется | Язык выбирается по сумме `probability`, а не по числу упоминаний: у всех языков упоминаний поровну |
...
Качество на терминах: специальная лексика искажается — «year pillar» → «yepila», «epila»,
«epilogue»; «palaces» → «policies»; «grandparents» → «grand talents». Глоссарий перевода это не
лечит: он применяется после распознавания. Открытый вопрос — есть ли у SpeechKit v3 словарь
подсказок; запасной путь — `--asr whisper` (см. `HANDOFF.md`, раздел 5).

## Реальное поведение MAX_DURATION

Пять наблюдений с двух прогонов (`<имя>_work/tts_fit.json`):

Колонка «Сжатие» = (естественная длительность − возврат) / естественная длительность.

| Символов | Естественная длительность | Запрошенный лимит | Вернул сервис | После обрезки тишины | Сжатие |
|---|---|---|---|---|---|
| 49 | 2555 мс | 2360 мс | 2352 мс | 2021 мс | 8 % |
| 208 | 12 626 мс | 12 100 мс | 12 047 мс | 11 562 мс | 5 % |
| 214 | 13 015 мс | 12 079 мс | **12 186 мс** | 11 712 мс | 6 % |
| 196 | 12 936 мс | 12 421 мс | 12 372 мс | 12 057 мс | 4 % |
| 240 | 14 391 мс | 12 149 мс | 12 012 мс | 11 568 мс | **16 %** |

`MAX_DURATION` работает, но это не жёсткая граница: в одном случае из пяти сервис вернул на 107 мс
(0,9 %) больше запрошенного. Перерасход не теряется: `build_voice_track` сдвигает следующую фразу,
и на плотных репликах сдвиг накапливается. Как звучит речь при сжатии до 16 %, пользователь пока не
оценил. Строку `duration` программа шлёт только для фраз не длиннее 250 символов: более длинные она
отправляет в `unsafeMode`, а два эти поля в запросе не совмещает.

Стоимость: 60 с в стандартном режиме программа оценила в 0,61 ₽ = 60 / 15 × 0,1515 — без множителя
за два канала, который предписывают тарифы. Фактическое списание со счётом не сверялось.

Предельное сжатие не проверено: лимит считается как `max(окно, длительность / --max-speed)`,
поэтому при широком окне он не опускается ниже окна, и `--max-speed 3` ничего не меняет — во всех
пяти наблюдениях запрошенный лимит равен окну. Чтобы получить жёсткий случай, нужна фраза, чьё
окно до следующей реплики само по себе мало.
