...
| Качество на терминах | Специальные термины искажаются: «year pillar» → «yepila», «epila», «epilogue»; «palaces» → «policies»; «grandparents» → «grand talents». Глоссарий перевода это не лечит — он применяется после распознавания | Открытый вопрос: есть ли у SpeechKit v3 словарь подсказок; запасной путь — `--asr whisper` |
...
(0,9 %) больше запрошенного. Сжатие до 16 % речь выдерживает. Строку `duration` программа шлёт
только для фраз короче 250 символов: длинные идут в `unsafeMode`, где подгонка недоступна.
